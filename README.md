...
Vortex 用的是**软部署**：它把模组文件放在自己的仓库里
（`%APPDATA%\Vortex\<游戏>\mods\<模组>\`），然后在游戏目录里建**符号链接**指过去。

所以有两件事要清楚：

- **游戏目录里的模组文件显示 0 字节是正常的**，那不是空文件，是符号链接。
  用 `dir /AL` 能看到 `<SYMLINK>` 和目标路径；用 `Get-Item` 读 `.Length` 只会得到 0，
  别把这个当成文件损坏。（`Get-Content` 读得到内容是正常的，它穿透了链接。）
- **在游戏目录里改文件，实际改的是 Vortex 仓库里那份**，两者是同一个文件。
  但 Vortex 下次 Purge / Deploy 时会从模组压缩包重新解压，手工改的译文就会被盖回去。

模板组一更新、或者你在 Vortex 里点了 Purge，译文就可能没了。所以这里把译文单独存一份，
并做成一条命令就能还原——数据在 `games/<游戏>/i18n/`，任何时候都能重建目标文件。
